ouchtree <- function (nodes, ancestors, times, labels = as.character(nodes)) {
  ...
  if (length(ancestors) != n)
    stop("invalid tree: 'ancestors' must have the same length as 'nodes'")
  if (length(times) != n) 
    stop("invalid tree: 'times' must have the same length as 'nodes'")
  if (length(labels) != n)
    stop("invalid tree: 'labels' must be the same length as 'nodes'")
